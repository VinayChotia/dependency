...
# class ApiExecutionFlowView(APIView):
...
#         response = HttpResponse(graph_image, content_type='image/png')
        
#         response['Content-Disposition'] = 'attachment; filename="execution_flow.png"'
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for endpoint in endpoints:
                ...
                if path not in dependency_list:
                    dependency_list[path] = dependencies

            return Response(dependency_list, status=status.HTTP_200_OK)

        except Exception as e:
            return Response({'error': f'Error exporting data: {e}'}, status=status.HTTP_400_BAD_REQUEST)
